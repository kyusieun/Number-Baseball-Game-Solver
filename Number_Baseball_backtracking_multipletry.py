...
def RemoveElements(list, nums, strike, ball):
    ...
    for _ in range(len(list)):
        tempStrike, tempBall = GetStrikeAndBall(list[i], nums, strike, ball)
...
def GetStrikeAndBall(curList, setList, strike, ball): # 중간에 tempStrike > strike or tempBall > ball인 순간에 바로 탈출하는 루트 추가
    tempStrike = 0
    tempBall = 0
    for i in range(4):
        for j in range(4):
            if ( i == j and curList[i] == setList[j]):
                ...
                if (tempStrike > strike): return tempStrike, tempBall
                break
            elif ( curList[i] == setList[j] ):
                tempBall = tempBall + 1
                if (tempBall > ball): return tempStrike, tempBall
                break
